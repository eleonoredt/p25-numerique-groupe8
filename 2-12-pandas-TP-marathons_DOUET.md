...
df #semble contenir les memes valeurs
```
...
df_1971.shape == (3, 4) and df_1971.seconds.max() == 8574  # c bon mais avant il y avait False car une colonne Unamed en trop
```
...
```{code-cell} ipython3
# votre code
count_by_city=df['city'].value_counts().sort_index()
count_by_city
```
...
```{code-cell} ipython3
# votre code
mean_city= df.groupby('city')['seconds'].mean()
mean_city
```
...
#on voit que on a les memes chiffres
```
...
```{code-cell} ipython3
# à vous # on utilise la même idée que au dessus
...
```{code-cell} ipython3
# votre co
df=df.drop(columns= ['hour', 'minute', 'second'] ) # en mettant columns on lui indique qu'il doit supprimer des colonnes et les crochets permettent d'en mettre plusieurs
df
...
temps=df['duration'].dt.components #on crée les colonnes
df[['hour', 'minute', 'second']] = temps[['hours', 'minutes', 'seconds']] #on les met dans df
df
```
