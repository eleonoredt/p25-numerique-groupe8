...
```{code-cell} ipython3
type(df['Pclass']) #c ma data frame, c une series
```
...
fi=pd.read_csv('data/petit-titanic.csv')
pd.DataFrame.head
fi.head()
help(pd.read_csv) #c des points virgules et pas des virgules, pas d'entete, 
```
...
colonnes= ['PassengerId', 'Survived', 'Pclass', 'Name', 'Sex', 'Age', 'SibSp',
          'Parch', 'Ticket', 'Fare', 'Cabin', 'Embarked']
fic= pd.read_csv("data/petit-titanic.csv", sep=";", header=None, names=colonnes)
fic.head()
```
...
```{code-cell} ipython3
# votre code
fic_age_fare=fic[['Age', 'Fare']]
fic_age_fare.describe()
```
...
# votre code
len(fic) #les lignes
len(fic.columns) #les colonnes
fic.shape #les 2 ensemble, lignes puis colonnes
```
...
```{code-cell} ipython3
# votre code
fic.columns = ['Identifiant', 'Survécu', 'Pclass',
           'Nom', 'Genre', 'Age', 'SibSp', 'Parch',
           'Ticket', 'Tarif', 'Cabine', 'Embarquement']
fic
```
...
```{code-cell} ipython3
# votre code
fich=fic.set_index('Identifiant')
fich
```
