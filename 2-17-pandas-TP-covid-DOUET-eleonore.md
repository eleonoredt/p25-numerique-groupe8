...
global_df2 = pd.concat([pd.DataFrame(data).assign(country=country) for country, data in by_country.items()])
global_df2
```
...
# votre code
dfs = []
for countries, values in by_country.items():
    df_countries = pd.DataFrame(values)
...
```{code-cell} ipython3
# avec set_index, on rajoute sort_index pour que les dates soient dans l'ordre
clean_df2 = global_df.set_index(['country', 'date']).sort_index()
...
```{code-cell} ipython3
# votre code, on utilise loc pour acceder aux valeurs, et on met des parenthèse dans les crochets car il s'agit d'un tuple
...
```{code-cell} ipython3
# votre code, on veut France et Italy il s'agit de 2 pays différents donc on prend comme uen liste les 2 pays
...
```{code-cell} ipython3
df3c = clean_df.loc[['France','Italy','Germany']][['confirmed','deaths']] #on a extrait les données des 3 pays
df3c.plot(rot=45)
```

```{code-cell} ipython3
#la on a une data frame à multiindex maintenant on voudrait faire une data frame index simple avec 6 colonnes deathIT, confirmedIT... pour chaque pays
#pour cela on doit donc passer de multi index à index simple
df3 = df3c.unstack(level=0) 
df3
```

```{code-cell} ipython3
#du coup il n'y a plus qu'à plotter
df3.plot(rot=45)
```

```{code-cell} ipython3
# votre code A NE PAS FAIRE, 1e essai
df3d = clean_df.loc[['France','Italy','Germany']][['confirmed','deaths']]
#la on a une data frame à multiindex maintenant on voudrait faire une data frame index simple avec 6 colonnes deathIT, confirmedIT... pour chaque pays
#pour cela on doit donc passer de multi index à index simple
df3 = df3d.unstack(level=0) 
df2 = pd.DataFrame() #premier essai, trop complique et trop sepcifique a cette dataframe
df2['confFR']=df3[('confirmed',  'France')]
df2['deathFR']=df3[('deaths',  'France')]
df2['confIT']=df3[('confirmed',  'Italy')]
df2['deathIT']=df3[('deaths',  'Italy')]
df2['confGE']=df3[('confirmed',  'Germany')]
df2['deathGE']=df3[('deaths',  'Germany')]
df2
```
...
```{code-cell} ipython3
def extract(df,pays, mesures, debut, fin): 
    df_fin = df.loc[pays, mesures].reset_index() 
    df_fin['date'] = pd.to_datetime(df_fin['date']) 
    df_fin = df_fin[(df_fin['date'] >= debut) & (df_fin['date'] <= fin)] 
    df_fin = df_fin.set_index(['date','country']).sort_index() 
    df_fin = df_int.unstack(level=1) 
    df_fin.columns = [f"{mesure}_{country}" for mesure, country in df_fin.columns] 
    df_fin = df_fin.sort_index() 
    df_fin = df_fin.apply(pd.to_numeric) 
    return df_fin
```
...
```{code-cell} ipython3
# les multi index nous ont montré que ca pouvait rendre des données plus lisibles on s'est rendu compte qu'on pouvait les fepresenter de maniere interessante. 
#notamment les fonctions comme extract permettent apres de representer plsr donées plus facilement
```
